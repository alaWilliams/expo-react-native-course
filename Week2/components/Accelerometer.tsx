// I used the existing accelerometer code and implemented myself code to detect shakes and increment a counter each time a shake is detected.
...
subscription.current = Accelerometer.addListener(({ x, y, z }) => {
  ...
  const value = Math.pow(x,2) + Math.pow(y, 2) + Math.pow(x, 2)
  const magnitude = Math.sqrt(value);
  console.log(magnitude.toFixed(3))

  const shakeStrength = magnitude - 1;


  if (shakeStrength > 2) {
    setIsShaking(true);
    setCounter(counter => counter + 1);
    console.log('SHAKE detected:', shakeStrength.toFixed(2));
  } else {
    setIsShaking(false);
  }
});
